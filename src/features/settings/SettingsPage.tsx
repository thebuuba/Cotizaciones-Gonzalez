...
export function SettingsPage() {
  ...
  return <div className="settings-page">
    ...
    <PwaInstallCard />
    <div className="settings-row"><div><Cloud aria-hidden="true" /><span><strong>Respaldo en la nube</strong><SyncBadge state={sync.state} /></span></div><div className="settings-actions"><button className="button button--quiet" type="button" onClick={() => void sync.syncNow()}><RefreshCw aria-hidden="true" />Sincronizar</button>{sync.signOut && <button className="icon-button icon-button--danger" type="button" aria-label="Cerrar sesión" onClick={() => void sync.signOut?.()}><LogOut aria-hidden="true" /></button>}</div></div>
    </div></section>
