...
export function QuotationDetailPage({ snapshot, onStatusChange }: {
  ...
  return <div className="quotation-detail">
    ...
    <section className="detail-summary"><small>{snapshot.quotation.number}</small><h1>{snapshot.quotation.clientName}</h1><p>{snapshot.quotation.clientAddress}</p><strong>{formatMoney(totals.totalMinor)}</strong><label>Estado de la cotización<select value={snapshot.quotation.status} onChange={(event) => void onStatusChange(event.target.value as QuotationStatus)}><option value="draft">Borrador</option><option value="sent">Enviada</option><option value="approved">Aprobada</option><option value="rejected">Rechazada</option></select></label></section>
    <section className="detail-materials"><h2>Materiales</h2>{snapshot.materialItems.map((item) => <div key={item.id}><span>{item.description}</span><strong>{formatMoney(calculateQuotationTotals([item], 0).materialsMinor)}</strong></div>)}<div><span>Mano de obra</span><strong>{formatMoney(snapshot.quotation.laborMinor)}</strong></div></section>
