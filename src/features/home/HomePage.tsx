...
import { calculateTotals, formatMoney } from '../../domain/money'
...
export function HomePage({ businessName, quotations, syncState }: { businessName: string; quotations: QuotationSnapshot[]; syncState: SyncState }) {
  ...
  const total = visible.reduce((sum, item) => sum + calculateTotals(item.workItems.map((work) => work.priceMinor), item.quotation.discount).totalMinor, 0)
  ...
  return <div className="dashboard">
    ...
    <section className="total-card"><span>Total cotizado este mes</span><strong>{formatMoney({ currency: 'DOP', amountMinor: total })}</strong></section>
    ...
    <section className="recent"><h3>Cotizaciones recientes</h3>{quotations.slice(0, 3).map((item) => <article className="quote-card" key={item.quotation.id}><div><small>{item.quotation.number}</small><h4>{item.quotation.projectName}</h4><span>{item.client.name}</span></div><div className="quote-card__aside"><StatusBadge status={item.quotation.status}/><strong>{formatMoney({ currency: item.quotation.currency, amountMinor: calculateTotals(item.workItems.map((work) => work.priceMinor), item.quotation.discount).totalMinor })}</strong></div></article>)}</section>
  </div>
}
